...
def plot_slices(
    ...
    if dim_reduction_method == "tsne":
        reducer = TSNE(n_components=2, perplexity=30, random_state=42, verbose=1)   
...
# Prepare unique colors for each slice index
    ...
    slice_color_map = {slice_idx: colors[i] for i, slice_idx in enumerate(unique_slices)}
    ...
    for slice_idx in unique_slices:
        ...
        error_indices = np.where(slice_targets != slice_pred_labels)[0]
        correct_indices = np.where(slice_targets == slice_pred_labels)[0]

        # Plot correct samples
        plt.scatter(
            slice_embeddings[correct_indices, 0],
            slice_embeddings[correct_indices, 1],
        ...
        # Plot error samples
        plt.scatter(
            slice_embeddings[error_indices, 0],
            slice_embeddings[error_indices, 1],
            color=slice_color_map[slice_idx],
            marker='o',  # X shape for error samples
            # edgecolor='black',
            # linewidth=1,
            alpha=0.8
        )
